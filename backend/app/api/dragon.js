...
router.put('/update', (req, res, next) => {
    ...
    .then(() => res.json({message: 'Successful update'}))
    .catch(error => next(error))
})
...
router.post('/buy',  (req, res, next) => {
    ...
    .then(dragon => {
        if(dragon.saleValue !== saleValue){
            throw new Error('Sale value is incorrect')
        }
        if(!dragon.isPublic){
    ...
    .then(({accountId}) => {
        if(accountId === buyerId){
            throw new Error('Cant buy your own dragon!')
        }
...
router.post('/mate', (req, res, next) => {
    ...
    if(matronDragonId === patronDragonId) throw new Error('Cannot breed the same dragon!')
    ...
    .then(dragon => {
        if(!dragon.isPublic) throw new Error('This dragon must be public')
        patronDragon = dragon
    ...
    .then(({account, authenticated}) => {
        ...
        if(patronSireValue > account.balance){
            throw new Error('Sire Value exceed balance')
        }
        matronAccountId = account.id
        return AccountDragonTable.getAccountDragons({dragonId: patronAccountId})
    })
    .then(({accountId}) => {
