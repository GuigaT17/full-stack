...
class DragonTable {
    ...
    static getDragon({dragonId}){
        return new Promise((resolve, reject) => {
            pool.query('SELECT birthdate, nickname, "generationId", "isPublic", "saleValue", "sireValue" FROM dragon WHERE dragon.id = $1',
            [dragonId],
            (error, response) => {
                if(error) return reject(error)
    ...
    static updateDragon({dragonId, nickname, isPublic, saleValue, sireValue}){
        ...
        const validQueries = Object.entries(settingsMap).filter(([key, value]) => {
            if(value !== undefined){
                return new Promise((resolve, reject) => {
                    pool.query(
                        `UPDATE dragon SET "${key}" = $1 WHERE id = $2`,
                        [value, dragonId],
                        (error, response) => {
                            if(error) return reject(error)
                            resolve()
